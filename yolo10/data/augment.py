...
from yolo10.utils import LOGGER, colorstr
from yolo10.utils.checks import check_version
from yolo10.utils.instance import Instances
from yolo10.utils.metrics import bbox_ioa
from yolo10.utils.ops import segment2box, xyxyxyxy2xywhr
from yolo10.utils.torch_utils import TORCHVISION_0_10, TORCHVISION_0_11, TORCHVISION_0_13
...
class Albumentations_v10:
    ...
    def __init__(self, p=1.0):
        ...
        # Transforms
        T = [
                # Simulate slight blur (avoid setting too high to prevent loss of small text)
                A.MotionBlur(blur_limit=3, p=0.2),          # Simulate camera shake blur
                A.GaussNoise(var_limit=(10.0, 30.0), p=0.2), # Add light noise

                # Color variations: adapt to different cameras/scans
                A.HueSaturationValue(hue_shift_limit=5, sat_shift_limit=10, val_shift_limit=10, p=0.3),

                # Slight brightness and contrast adjustments
                A.RandomBrightnessContrast(brightness_limit=0.1, contrast_limit=0.1, p=0.3),

                # Image compression simulation (simulate low-quality screenshots/compressed images)
                A.ImageCompression(quality_lower=80, quality_upper=100, p=0.2),

                # Small affine transformations (rotation, translation, scaling) to improve robustness while preserving text structure
                A.Affine(
                    scale=(0.95, 1.05),
...
def v10_transforms(dataset=None, imgsz=None, hyp=None, stretch=False):
    """Return Albumentations_v10-only transforms in v8_transforms-style Compose structure."""
    return Compose([
